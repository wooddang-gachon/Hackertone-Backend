...
## 🏆 Phase 5: 랭킹 시스템 (Ranking System) - [Completed]
**목표**: 분야별 전문가를 시각화하고 사용자들의 동기부여를 강화합니다.

1.  **카테고리별 랭킹 API** (`POST /rankings/:category`)
    - `point`, `exercise`, `study`, `music`, `game`, `clean` 기준 정렬.
    - 상위 10명(기본값) 랭킹 리스트 반환.
2.  **내 순위 실시간 계산**
    - `RANK()` 윈도우 함수를 활용하여 전체 사용자 중 현재 로그인한 유저의 순위와 점수를 함께 반환.
